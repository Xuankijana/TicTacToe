...
public class Board {
	...
	public Board() {
	 ...
	 //NOTE: initialise the cells array using ROWS and COLS constants 
	...
	public boolean isDraw() {
		...
		// NOTE: Check whether the game has ended in a draw. 
		// NOTE: Use a nested loop (see the constructor for an example). 
		// Check whether any of the cells content in the board grid are Player.Empty. If they are, it is not a draw.
		// NOTE: Return false if it is not a draw, return true if there are no empty positions left
		   for (int row = 0; row < GameMain.ROWS; ++row) {
	            for (int col = 0; col < GameMain.COLS; ++col) {
	...
	public boolean hasWon(Player thePlayer, int playerRow, int playerCol) {
		...
		if(    cells[playerRow][0].content == thePlayer 
		 ...
		 // NOTE: Check if the player has 3 in the playerCol.
		 // NOTE: Use the row code above as a starting point, remember that it goes cells[row][column] 
		...
		// NOTE: Check the diagonal in the other direction
